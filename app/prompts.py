...
def story_generation_system_prompt(style_instructions: str, writing_rules: str) -> str:
    return dedent(
        ...
        - 用户写的“这一章希望发生什么”必须真实发生在正文里，不能只停留在标题、摘要、暗示、旁白计划或未兑现的铺垫里。
        - 如果用户要求了多个推进点，正文至少要覆盖其中的大部分核心动作，不能只挑最容易写的一小部分。
...
def story_generation_user_prompt(
    *,
    ...
    return dedent(
        ...
        - 用户本次写作意图里明确要求发生的内容，必须落实到正文实际场景与行动里。
        - 不允许把用户要求的关键推进点改写成“准备去做”“想起这件事”“打算以后再处理”来敷衍覆盖。
