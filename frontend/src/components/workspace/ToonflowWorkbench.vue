    ...
    <main class="toon-stage">
      ...
      <section v-else-if="activeModule === 'projects'" class="toon-projects">
        ...
        <div v-if="visibleProjects.length" class="toon-project-grid">
          <article v-for="project in visibleProjects" :key="project.id" tabindex="0" role="button" :aria-label="`打开项目 ${project.title}`" @click="openProject(project.id)" @keydown.enter.self.prevent="openProject(project.id)" @keydown.space.self.prevent="openProject(project.id)">
      ...
      <section v-else class="toon-workbench">
        <aside class="toon-inspector">
          ...
          <div class="toon-inspector__block"><strong>下一步</strong><p v-if="!selectedProject">先选择一个项目。</p><p v-else-if="!selectedProject.world_brief && !selectedProject.writing_rules">先补充故事资料与改编要求，再进入后续生成。</p><p v-else-if="!storyboards.length">故事资料已进入项目，下一步需要生成或导入分镜。</p><p v-else-if="!mediaAssets.length">分镜已经存在，下一步准备角色资产和镜头首帧。</p><p v-else-if="!videoTasks.length">资产已经进入生产区，下一步执行预检并创建视频任务。</p><p v-else>视频任务已经开始，持续关注右侧运行记录和质量结果。</p></div>
...
button:focus-visible, input:focus-visible, textarea:focus-visible, select:focus-visible { outline: 2px solid color-mix(in oklab, var(--toon-rose-deep) 66%, white); outline-offset: 2px; }
button:disabled { cursor: wait; opacity: .48; }
.toon-button--dark { border-color: transparent; background: var(--toon-rose-deep); color: white; }
...
.toon-rail button.active, .toon-rail__brand { background: var(--toon-rose-deep); color: white; box-shadow: 0 10px 22px color-mix(in oklab, var(--toon-rose) 30%, transparent); }
...
.toon-topbar nav button.active { border-color: transparent; background: var(--toon-rose-deep); color: white; }
...
.toon-storyboard-list button.active { border-color: transparent; background: var(--toon-rose-deep); color: white; }.toon-storyboard-list small { opacity: .66; }
